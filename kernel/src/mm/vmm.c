...
static bool clone_table_cow(struct user_address_space *dst,
                            struct user_address_space *src,
                            uint64_t dst_table_pa,
                            uint64_t src_table_pa,
                            unsigned level) {
    ...
    for (size_t i = 0; i < 512; ++i) {
        ...
        if (!clone_table_cow(dst, src, child_pa, entry & PTE_ADDR_MASK, level + 1)) {
            return false;
        }
    }
    return true;
...
bool vmm_handle_cow_fault(struct user_address_space *as, uint64_t va) {
    ...
    if (pte == NULL || (*pte & (PTE_VALID | PTE_COW)) != (PTE_VALID | PTE_COW)) {
        return false;
    }
    ...
    kmemcpy((void *)(uintptr_t)(as->hhdm_offset + new_pa),
            (const void *)(uintptr_t)(as->hhdm_offset + old_pa),
            PAGE_SIZE);

    *pte = 0;
    vmm_flush_user_va(va);
    uint64_t new_entry = leaf_with_ap((old_entry & ~PTE_ADDR_MASK & ~PTE_COW) |
                                          (new_pa & PTE_ADDR_MASK),
                                      PTE_AP_USER_RW);
    *pte = new_entry;
...
bool vmm_copy_to_user(const struct user_address_space *as, uint64_t dst, const void *src, size_t len) {
    ...
    for (size_t i = 0; i < len; ++i) {
        if (user_to_phys_checked(as, dst + i, VMM_ACCESS_WRITE) == 0 &&
            !vmm_handle_cow_fault(mutable_as, dst + i)) {
            return false;
        }
    }
